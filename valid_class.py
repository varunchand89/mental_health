...
class valid:
    def __init__(self,dt):
        self.dt = dt
    def val(self):
        self.dt = self.dt[self.dt['Dietary Habits'].isin(valid_values)]
        self.dt = self.dt[self.dt['Degree'].isin(valid_degree)]
        self.dt = self.dt[self.dt["City"].isin(valid_city)]
        self.dt = self.dt[self.dt["Profession"].isin(valid_professions)]
        self.dt = self.dt[~self.dt["Name"].isin(no_valid_name)]
        self.dt = self.dt[~self.dt["Sleep Duration"].isin(no_valid_sleep)]
        return self.dt